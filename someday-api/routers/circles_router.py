...
async def create_circle(
    ...
    if status == 201:
        background_tasks.add_task(track_product_event, "circle_created", "circles")
    return create_response(status, result)
...
async def update_circle(
    ...
    infologger.info(
        f"PATCH /circles/{circle_id} | user_id={current_user['sub']} payload={request.model_dump()}"
    )
...
async def upload_photo(
    circle_id: str, file: UploadFile = File(...), current_user: dict = Depends(jwt_required)
):
    """Upload a circle photo (members only; pre-resized image ≤2MB)."""
    infologger.info(
        f"POST /circles/{circle_id}/photo | user_id={current_user['sub']} type={file.content_type}"
    )
    content = await file.read()
    status, result = handler.upload_photo(
        circle_id, current_user["sub"], content, file.content_type or ""
    )
    return create_response(status, result)
...
async def set_member_role(
    circle_id: str, target_id: str, request: RoleRequest, current_user: dict = Depends(jwt_required)
):
    infologger.info(
        f"PATCH /circles/{circle_id}/members/{target_id} | actor={current_user['sub']} role={request.role}"
    )
    status, result = handler.set_member_role(
        circle_id, current_user["sub"], target_id, request.role
    )
    return create_response(status, result)
...
async def remove_member(circle_id: str, target_id: str, current_user: dict = Depends(jwt_required)):
    infologger.info(
        f"DELETE /circles/{circle_id}/members/{target_id} | actor={current_user['sub']}"
    )
