...
async def list_intents(
    ...
    status, result = _handler.list_intents(
...
async def create_intent(
    ...
    # Unfurl URL if provided — non-blocking on failure (warning logged in unfurl_handler)
    link_meta = None
    if request.url:
        link_meta = fetch_link_meta(request.url)

    status, result = _handler.create_intent(circle_id, request, current_user["sub"], link_meta)
    return create_response(status, result)
...
async def get_intent(intent_id: str, current_user: dict = Depends(jwt_required)):
    ...
    status, result = _handler.get_intent(intent_id, current_user["sub"])
    return create_response(status, result)
...
async def update_intent(
    ...
    status, result = _handler.update_intent(intent_id, request, current_user["sub"])
    return create_response(status, result)
...
async def delete_intent(intent_id: str, current_user: dict = Depends(jwt_required)):
    ...
    status, result = _handler.delete_intent(intent_id, current_user["sub"])
    return create_response(status, result)
...
async def toggle_reaction(intent_id: str, current_user: dict = Depends(jwt_required)):
    ...
    status, result = _handler.toggle_reaction(intent_id, current_user["sub"])
    return create_response(status, result)
...
async def toggle_boost(intent_id: str, current_user: dict = Depends(jwt_required)):
    ...
    status, result = _handler.toggle_boost(intent_id, current_user["sub"])
    return create_response(status, result)
